...
* `-r <runtimeID>`:  Build an executable for the specific runtime.  Tested choices include `win-x64`, and `linux-x64`.
...
* `-p:LinuxVersion=<version>`:  Specify a version of Linux for which we've provided prebuilt dependencies (see subdirectories of Client\Unmanaged\binary\linux-x64)

Example 1 (making a compressed, self-contained release build for Windows x64):
```
cd Client
...
Example 2 (compressed, self-contained release build for Linux, Ubuntu 24.04):
```
cd Client
dotnet publish -c Release -r linux-x64 -p:LinuxVersion=Ubuntu-24.04 -p:SelfContainedRelease=true

```
The "published" output here would end up in `(repo root)\Publish\Ubuntu-24.04_SelfContained`.